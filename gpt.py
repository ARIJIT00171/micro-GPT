...
class CausalSelfAttention(nn.Module):
    ...
    def apply_rotary_emb(self, xq: torch.Tensor, xk: torch.Tensor, T: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        pos_emb = torch.cat((freqs, freqs), dim=-1).to(xq.device)  # Shape: (1, 1, T, dim)
        
        # Split pos into sin and cos components, repeating each to match xq and xk dimensions
        pos_sin = pos_emb.sin().unsqueeze(0).unsqueeze(0)
        pos_cos = pos_emb.cos().unsqueeze(0).unsqueeze(0)
        ...
        xq_rot = torch.stack(
        [xq_even * pos_cos[..., ::2] - xq_odd * pos_sin[..., ::2],
         xq_even * pos_sin[..., ::2] + xq_odd * pos_cos[..., ::2]],
        dim=-1,).flatten(-2)
        xk_rot = torch.stack(
        [xk_even * pos_cos[..., ::2] - xk_odd * pos_sin[..., ::2],
         xk_even * pos_sin[..., ::2] + xk_odd * pos_cos[..., ::2]],
        dim=-1,).flatten(-2)
    ...
    def forward(self, x):
        ...
        if self.use_flash_attn:
            attn_mask = self.mask[:, :, :T, :T].to(dtype=torch.bool)
            y = F.scaled_dot_product_attention(q, k, v, dropout_p = self.config.attn_pdrop, attn_mask = attn_mask)
        else:
            ...
            att.masked_fill_(causal_mask == 0, -torch.inf)
...
class TransformerDecoderBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.layer_att = CausalSelfAttention(config)
        self.layer_norm_2 = RMSNorm(config.n_embd)
        self.layer_mlp = nn.Sequential(nn.Linear(config.n_embd, 4 * config.n_embd), 
                                       BERTGELU(), 
        ...
        for _ in range(max_new_tokens):
            ...
            if not do_sample:
                ...
                if top_k is not None:
                    ...
                    k_value = top_k_logits[:, -1, None]
                    probs = torch.where(logits < k_value, torch.zeros_like(probs), probs)
                    #pass
                ...
                if top_p is not None:
                    ...
                    mask[:, 1:] = mask[:, :-1].clone()
                    mask[:, 0] = False
                    probs[mask.scatter(1, sorted_indices, mask)] = 0.0
                    #pass
